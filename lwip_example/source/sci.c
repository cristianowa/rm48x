...
void sciDisplayTextInternal(sciBASE_t *sci, uint8_t *text)
{
    while(*text != '\0')
    {
...
{
    uint32_t vec = scilinREG->INTVECT1;
...
/* USER CODE END */
    ...
    case 11:
        /* receive */
        {   uint32_t byte = scilinREG->RD;

            if (g_sciTransfer[1].length > 0)
            {
                *g_sciTransfer[1].data++ = byte;
                g_sciTransfer[1].length--;
                if (g_sciTransfer[1].length == 0)
                {
    ...
    default:
        ...
        scilinREG->FLR = scilinREG->SETINTLVL & 0x07000303;
         break;
    }
/* USER CODE BEGIN (27) */
/* USER CODE END */
...
/* USER CODE BEGIN (28) */
  uint32_t rx_int =0;  
  rx_int = sciReceiveByte(scilinREG); 
  sci_transfer_buffer[0] = rx_int & 0xFF;
  sci_transfer_buffer[1] = rx_int & 0xFF00;
  sci_transfer_buffer[2] = rx_int & 0xFF0000;
  sci_transfer_buffer[3] = rx_int & 0xFF000000;
  g_sciTransfer[1].length++;
/* USER CODE END */
