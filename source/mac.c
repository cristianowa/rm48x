...
int macInit()
{
          ...
          print_line("");
          #ifdef __MAC_DEBUG__
          print_line(" Dumping EMAC Registers : ");
          volatile int *ptr;
          ptr = (int *) EMAC_BASE;
          dump_memory((void*)ptr,163);
          print_line(" Dumping MDIO Registers ");          
          ptr = (int *) MDIO_BASE;
          dump_memory((void*)ptr,14);          
          #endif
          return 0;
}
...
int eth_transmit(uint8_t* buf,uint32_t length)
{
  ...
  else
  {
      ...
      sprintf(&text_buf,"tx buf addr %x",&tx_buffer);
      print_line(text_buf);
